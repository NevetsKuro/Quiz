...
public class User1 {
    ...
    public boolean initialiseUser() {
        ...
        if (isloggedin) {
            ...
            ssql = "select user_id as emp_code,user_name as emp_name,designation,email_id as email "
                    + "from hindi_mst_user "
                    + "where "
                    + " lpad(user_id,8,0)=lpad('" + getPfnumber() + "',8,0)";
    ...
    public int sendreminderEmail(int report_yymm) {
        // usage today();
        String ssql = "";
        Statement stmt;
        sErrorMsg = "";
        ...
        try {
            ...
            ssql = "insert into maildb (report_id,mailrecipient,subject,mailtext,mailsender)"
                    + "( select '00',email_id,mailsubject,mailtext,'hindimailer@indianoil.in'"
    ...
    public int generateReportNew(int report_yymm, String meetingdate, String meetingdesc) {
        // usage today();
        globals.DBService lib = new globals.DBService();
        Connection con = lib.getConnection();
        CallableStatement st;
